...
const regexImport = /import \{ useDemo \} from '@\/lib\/DemoContext';/;
const replaceImport = `import { useDemo } from '@/lib/DemoContext';\nimport { useState } from 'react';`;

// Add invoice state and handler
const regexState = /const \[trackingNumber, setTrackingNumber\] = useState\(''\);/;
const replaceState = `const [trackingNumber, setTrackingNumber] = useState('');
...
// Groups logic
const blockStart = '<div className="bg-rio-surface p-6 rounded-2xl shadow-sm border border-rio-border">';
const blockEnd = '</div>\n          </div>\n\n          <div className="space-y-6">';

const listBlockRegex = /<div className="md:col-span-2 space-y-6">\s*\{\/\* Order Details \*\/\}\s*<div className="bg-rio-surface p-6 rounded-2xl shadow-sm border border-rio-border">[\s\S]*?<\/div>\s*<\/div>\s*<div className="space-y-6">/;

const newListBlock = `<div className="md:col-span-2 space-y-6">
            {/* Order Details */}
            {(() => {
                        ...
                        {group.id !== 'main' && (
                          <div className="flex items-center gap-2 mt-2 w-full md:w-auto">
                            <input 
                              ...
                              className="border border-rio-border rounded-lg px-3 py-1.5 text-sm w-full md:w-40"
...
code = code.replace(listBlockRegex, newListBlock);

const printBlockRegex = /\{\(\(\) => \{[\s\S]*?const itemsToPrint = sortedItems\.filter\(item => printingSingle \? item\.id === printingSingle : true\);[\s\S]*?return \([\s\S]*?\}\)\(\)\}/;
...
code = code.replace(printBlockRegex, newPrintBlock);
