...
export default function ClienteDetalleAdminPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { customers, orders, updateCustomer } = useDemo();
        ...
        <div className="md:col-span-1 space-y-6">
          ...
          <div className="bg-rio-surface p-6 rounded-2xl shadow-sm border border-rio-border text-center relative overflow-hidden">
            ...
            <div className="mt-6 border-t border-rio-border pt-4 space-y-3">
              ...
              <button onClick={() => alert('Acción simulada: Editar descuento')} className="w-full py-2 text-[12px] font-bold text-rio-gold-dark bg-rio-gold-light/20 hover:bg-rio-gold-light/40 border border-rio-gold-light rounded-lg transition-colors flex justify-center items-center">
          ...
          <div className="bg-rio-surface p-5 rounded-2xl shadow-sm border border-rio-border space-y-2">
            ...
            <button onClick={() => alert('Acción simulada: Activar/Suspender')} className="w-full flex items-center p-3 text-sm font-semibold rounded-xl border border-rio-border hover:bg-rio-surface-muted transition-colors text-rio-ink">
            ...
            <button onClick={() => alert('Acción simulada: Reenviar invitación')} className="w-full flex items-center p-3 text-sm font-semibold rounded-xl border border-rio-border hover:bg-rio-surface-muted transition-colors text-rio-ink">
              <Mail className="w-4 h-4 mr-3 text-rio-gold-dark" />
              Reenviar Invitación
            </button>
            
            <button onClick={() => alert('Acción simulada: Restablecer credenciales')} className="w-full flex items-center p-3 text-sm font-semibold rounded-xl border border-rio-border hover:bg-rio-surface-muted transition-colors text-rio-ink">
              <Key className="w-4 h-4 mr-3 text-rio-warning" />
              Restablecer Acceso
            </button>

            <button onClick={() => alert('Acción simulada: Notificar requerimiento')} className="w-full flex items-center p-3 text-sm font-semibold rounded-xl border border-rio-border hover:bg-rio-surface-muted transition-colors text-rio-ink">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-rio-surface p-6 rounded-2xl shadow-sm border border-rio-border">
            <div className="flex justify-between items-center mb-4 border-b border-rio-border pb-3">
              ...
              <button onClick={() => alert('Acción simulada: Editar datos')} className="text-[11px] font-bold text-rio-gold-dark hover:text-rio-gold flex items-center uppercase tracking-wider">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium text-rio-ink">Bogotá (Mock)</p>
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t border-rio-border">
              <div>
                ...
                <p className="font-medium text-rio-ink">01/01/2026 (Mock)</p>
              </div>
              <div>
                <p className="text-[10px] uppercase font-bold text-rio-muted tracking-wider mb-1">Último Acceso</p>
                <p className="font-medium text-rio-ink">Hace 2 horas (Mock)</p>
              </div>
            </div>
          </div>
